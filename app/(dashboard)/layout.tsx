      ...
      <nav className="nav">
        ...
        <div className="nav-right">
          <div style={{ position: 'relative' }}>
            ...
            {mostrarNotifs && <PanelNotificaciones />}
          </div>
          <div className="nav-usuario">Hola, <span>{nombre}</span></div>
          <button className="btn-salir" onClick={cerrarSesion}>SALIR</button>
        </div>
        <div className="nav-centro-movil" style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', display: 'none', fontSize: '0.8rem', color: '#4a4038', fontFamily: 'DM Sans, sans-serif' }}>
          Hola, <span style={{ color: '#c4a520', fontWeight: 500 }}>{nombre}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <div style={{ position: 'relative', display: 'none' }} className="notif-btn-movil">
            <button className="notif-btn" onClick={() => setMostrarNotifs(!mostrarNotifs)} style={{ color: '#1a1612', fontSize: '1.3rem' }}>
              🔔
              {noLeidas > 0 && <span className="notif-badge">{noLeidas}</span>}
            </button>
            {mostrarNotifs && <PanelNotificaciones />}
